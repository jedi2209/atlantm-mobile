...
describe('Car order', () => {
    ...
    describe('Car Order Alert', () => {
        it('Car Order Alert must be loaded', async () => {
            await expect(element(by.text(ru.NewCarItemScreen.Notifications.buyType.title)).atIndex(0)).toExist();
            await element(by.label(ru.Base.cancel.toLowerCase())).atIndex(0).tap();
        });

        it('Car Order => click to WEB', async () => {
            await element(by.id(buttons.order)).tap();
            await expect(element(by.label(ru.NewCarItemScreen.makeOrder)).atIndex(0)).toExist();
        ...
        it('Car Order => in-APP order', async () => {
            ...
            await element(by.id(buttons.order)).tap();
            await expect(element(by.label(ru.NewCarItemScreen.sendQuery)).atIndex(0)).toExist();
    ...
    describe('Car Order Form', () => {
        ...
        it('Car Order Form Button must be hidden', async () => {
            await expect(element(by.id(dataConst.form.buttonSubmit))).toHaveValue('false');
        });
        ...
        it('Car Order Form Button must be showed', async () => {
            ...
            await waitFor(element(by.id(dataConst.form.buttonSubmit))).toHaveValue('true').withTimeout(1500);
        });
    });
});